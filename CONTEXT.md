...
- "the repo" was ambient — treepad acted on whichever repo the caller was standing in, and could only ever mean one at a time. Resolved: a caller names the repo it means. Standing in a repo is a convenience the CLI keeps, not what "the repo" is. See [ADR 0004](./docs/adr/0004-treepad-exposes-a-library-facade.md).
